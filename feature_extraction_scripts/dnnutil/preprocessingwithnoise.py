    ...
    with io.open(labelfile ,'r',encoding='iso-8859-15') as f:
        ...
        if len(new_align)>1:
            if new_align[-1]['end'] >= int(fileend):
                #print("Discard as we're over the end of the file (how?) %s" % l)
                conf.discard_counter += 1
                del new_align[-1]
...
def get_features( conf, audiofile, align,  preprocessors, cleanphonedata,  phoneclasses, phone_indices, 
    ...
    if training:
        speedups =  [0.95, 1.0, 1.05 ]
    else:
        speedups = [1.0]
        
    for speedup in speedups:

        preprocessor_string = os.path.basename(audiofile) + ' ' + str(speedup)
        ...
        if len(new_align) > 0:
            ...
            preprocessor_string += ' ' + preprocessor_key

            preprocessor = conf.preprocessing_scripts[ preprocessor_key ]

            preprocessed_audio = os.path.join(conf.tmp_dir,
                                            str(conf.tmpfilecounter)+"_"+str(speedup)+"_clean")

            if conf.debug:
                print ("Preprocessor : %s" % preprocessor['name'])
                
            if preprocessor['parameters'][0][1] - preprocessor['parameters'][0][0] > 0:
                param1 = str(random.randint( preprocessor['parameters'][0][0],
                                             preprocessor['parameters'][0][1]) )                
            else:
                param1 = str(preprocessor['parameters'][0][0])
                

            if preprocessor['parameters'][1][1] - preprocessor['parameters'][1][0] > 0:                    
                param2 = str(random.randint( preprocessor['parameters'][1][0],
                                                 preprocessor['parameters'][1][1]) )
            else:
                param2 = str(preprocessor['parameters'][1][0])
            
            preprocessor_string += '_%s_%s' % (param1, param2)

            if conf.debug:
                print (preprocessor_string)

            if 1 == 1:

                preprocess_progress = Popen([preprocessor['script'],
                                             audiofile,
                                             preprocessed_audio,
                                             str(speedup),
                                             param1,
                ...
                try:
                    audiodata = np.fromfile( preprocessed_audio, 'int16', -1)
                except:
                    print("\nPROBLEM!")
                    print("Could not find cleandata features: %s" % preprocessed_audio )
                    continue
                ...
                tmp_input=os.path.join(conf.tmp_dir,str(conf.tmpfilecounter)+"_in")
                tmp_output=os.path.join(conf.tmp_dir,str(conf.tmpfilecounter)+"_out")
                ...
                if True:
                    ...
                    try:
                        feature_array = feature_list.reshape([-1,conf.feature_dimension])
                    except:
                    ...
                    features_length_in_frames = feature_array.shape[0]


                cleanphonedata[ conf.lastframeindex : conf.lastframeindex + features_length_in_frames, :] = feature_array[:features_length_in_frames,:]
                ...
                if (feature_array.shape[0] < f_end / 16000 * conf.fs):
...
def extract_collection_and_save( conf, collection, training=True ):
    ...
    for r in recipefile.readlines():
        ...
        if conf.preprocessing_options:
            ...
            preprocessors = ['none','overdrive','babble', 'humming'] 
