...
for testround in range(12):
    ...
    if os.path.isfile(lsq_file):
        lsq_weight_array[testround,:] = np.loadtxt(lsq_file).reshape([-1])
    ...
    for category in ['native','good', 'ok', 'bad' ]:
        ...
        if len(test_samples) > 0:
            ...
            scores[testround][category]= (lsq_weight_array[testround,:] * leave_one_out_test_ranking_array[sub_test_samples,:]).sum(-1) - bonuspoints
    ...
    for category in ['native','good', 'ok', 'bad' ]:
        ...
        train_scores[testround][category]= (lsq_weight_array[testround,:] * leave_one_out_train_ranking_array[sub_test_samples,:]).sum(-1) - bonuspoints
        
        m=np.mean(train_scores[testround][category])
        st=np.std(train_scores[testround][category])
