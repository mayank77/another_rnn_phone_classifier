...
ranking_array = np.zeros([2300, 45,120])
score_array = np.zeros([2300])
...
for category in ['native','good', 'ok', 'bad' ]:
    ...
    for line in details:
        ...
        if currentsourcefile != sourcefile:                
            ...
            #print(os.path.basename(currentsourcefile))
            if (ranking_matrix).sum(-1).sum(-1) > 0:
                ranking_matrix /= (ranking_matrix).sum(-1).sum(-1)
            print ("Score: %f (should be around %i)\n" % ( (lsq_weights*ranking_matrix).sum(-1).sum(-1), points_per_phone[category])  )
        ...
        if phone != "sil":
            ...
            wanted = classes[rowcounter]

            ranking_matrix[ wanted, guess ] += 1

            print ("Wanted: %s -> %s Got: %s" % ( phone, classdict["_%i_"%wanted], classdict["_%i_"%guess] ) )
