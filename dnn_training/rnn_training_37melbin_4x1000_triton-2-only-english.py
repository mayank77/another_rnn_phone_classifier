...
logs_path = '../models/rnn%ix%i-learningrate%0.5f-dropout%0.1f-classbalance%0.1f-triton-a' % (num_layers, rnn_size, learning_rate, dropoutval, class_balancing)
LOG_DIR=logs_path
...
#                               [ "eval_fi", eval_fi_x, eval_fi_y, eval_fi_keys, eval_fi_writer, balanced_eval_fi_writer ],
                    ...
                    np.savetxt(os.path.join(LOG_DIR, 'cp%i_%s_confusion_matrix' % (i, testname)),
                               confmatrix,
                               fmt='%i',)

                    np.savetxt(os.path.join(LOG_DIR, 'cp%i_%s_id_y_and_prediction' % (i, testname)),
                               predictions,
                               fmt='%i',)

                    np.savetxt(os.path.join(LOG_DIR, 'cp%i_%s_id_y_and_activations' % (i, testname)),
                               activations,
                               fmt='%i',)
