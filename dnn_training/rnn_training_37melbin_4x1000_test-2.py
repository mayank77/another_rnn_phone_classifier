...
logs_path = '../models/rnn%ix%i-c' % (num_layers, rnn_size)
LOG_DIR=logs_path
#
        ...
        for i in range(total_batch):
            if ( (i-250) % 2000 == 0):
                ...
                for tests in [ [ "train_sample", train_sample_x, train_sample_y, train_sample_keys, train_sample_writer, balanced_train_sample_writer],
                    ...
                    for n in np.arange(0,testlen+eval_batch_size-1,eval_batch_size):
                        start=n
                        end=min(n+eval_batch_size, testlen)
                        ...
                        print ("activations slot shape")
                        print (activations[start:end,2:].shape)
                        '''
                    ...
                    np.savetxt(os.path.join(LOG_DIR, '%s_confusion_matrix' % (testname)),
                               confmatrix,
                               fmt='%i',)

                    np.savetxt(os.path.join(LOG_DIR, '%s_id_y_and_prediction' % (testname)),
                               predictions,
                               fmt='%i',)

                    np.savetxt(os.path.join(LOG_DIR, '%s_id_y_and_activations' % (testname)),
                               activations,
                               fmt='%i',)
                    ...
                    '''
                    sys.stderr.write("Acc: %0.2f/%0.2f Test: %s\t \n" % (biased_performance, balanced_performance, testname) )
